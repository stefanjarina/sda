...
@build-all:
        echo "Building for 'windows/amd64'..."
        $Env:GOOS="windows"; $Env:GOARCH="amd64"; go build -o target/{{ app_name }}-$($Env:GOOS)-$($Env:GOARCH).exe main.go
        echo "Building for 'windows/arm64'..."
        $Env:GOOS="windows"; $Env:GOARCH="arm64"; go build -o target/{{ app_name }}-$($Env:GOOS)-$($Env:GOARCH).exe main.go
        echo "Building for 'linux/amd64'..."
        $Env:GOOS="linux"; $Env:GOARCH="amd64"; go build -o target/{{ app_name }}-$($Env:GOOS)-$($Env:GOARCH) main.go
        echo "Building for 'linux/arm64'..."
        $Env:GOOS="linux"; $Env:GOARCH="arm64"; go build -o target/{{ app_name }}-$($Env:GOOS)-$($Env:GOARCH) main.go
